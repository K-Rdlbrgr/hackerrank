def arrayManipulation(n, queries):
    ...
    for first, last, value in queries:
        ...
        arr[first] += value
        # The first index after the last manipulated entry gets reduced by the same value in order to see where the incrementation ended, so the function can calculate the max later on.
        if (last != len(arr) - 1):
            arr[last + 1] -= value

    # Once the manipulation of the array is finished, the loop sums the values up from the start and updates the curretn maximum each time in case the count is actually larger than the result
    for number in arr:
        count += number
        if count > result:
            result = count
...
# This solution, while being shorter and more intuitive is not efficient enough to pass the time constraints of the challenge
