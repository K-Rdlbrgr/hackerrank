...
def sherlockAndAnagramsIntuitive(s):
    ...
    # Looping through the extracted substrings, comparing if two substrings are anagrams or not by using the Counter operator
    for index, sub in enumerate(substrings):
        for s in range(index+1, len(substrings)):
            # The counter operator checks if two substrings have the same amount of letters and consequently adds one to the counter if they do
